...
class WordReader
    ...
    func addTableRow blockIndex, rowData
        /*  Append a new row to the table at aBlocks[blockIndex].
            rowData: list of strings — one per column.
            Cells inherit basic structure (no colspan/bgColor).  */
        ...
        for cellText in rowData
            ...
            newRow + cell
        next
        aBlocks[blockIndex][:rows] + newRow
        return self

    func removeTableRow blockIndex, rowNum
        /*  Remove row rowNum (1-based) from the table at aBlocks[blockIndex].  */
        if blockIndex < 1 or blockIndex > len(aBlocks)  return self  ok
        if aBlocks[blockIndex][:type] != "table"        return self  ok
    ...
    # =========================================================================
    # Text replacement inside a block
    # =========================================================================
    ...
    # =========================================================================
    # Append helpers  (add blocks at end of aBlocks)
    # =========================================================================

    func appendEmptyParagraph
        block = [:type="paragraph", :text="", :runs=[], :align="",
                 :spaceBefore=0, :spaceAfter=0, :lineSpacing=0,
                 :lineSpacingPt=0, :lineRule="", :bgColor="",
                 :indent=0, :indentRight=0, :indentFirstLine=0,
                 :keepNext=false, :keepLines=false, :pageBreakBefore=false,
                 :outlineLvl=-1, :contextualSpacing=false,
                 :widowControl=true, :noHyphenate=false,
                 :bookmark="", :hasBorder=false, :borderStyle="",
                 :borderColor="", :borderSize=0, :borderSpace=0,
                 :borderSides=[]]
        aBlocks + block
        return self

    func appendHorizontalLine
        aBlocks + [:type="hline"]
        return self

    func appendHeading text, level
        if text  = NULL  text  = ""  ok
        if level = NULL  level = 1   ok
        block = [:type="heading", :text=text, :level=level, :runs=[],
                 :align="", :spaceBefore=0, :spaceAfter=0,
                 :numbered=false, :numStr="", :bookmark=""]
        aBlocks + block
        return self

    func appendParagraph text, options
        if text = NULL  text = ""  ok
        spBefore = 0
        spAfter  = 0
        align    = ""
        ...
        indent   = 0
        if isList(options)
            v = options[:spaceBefore]
            if isNumber(v)   spBefore = v  ok
            v = options[:spaceAfter]
            if isNumber(v)   spAfter  = v  ok
            v = options[:align]
            if v != NULL     align    = v  ok
            v = options[:bgColor]
            if v != NULL     bgColor  = v  ok
            v = options[:indent]
            if isNumber(v)   indent   = v  ok
        ok
        block = [:type="paragraph", :text=text, :runs=[], :align=align,
                 :spaceBefore=spBefore, :spaceAfter=spAfter,
                 :lineSpacing=0, :lineSpacingPt=0, :lineRule="",
                 :bgColor=bgColor, :indent=indent,
                 :indentRight=0, :indentFirstLine=0,
                 :keepNext=false, :keepLines=false, :pageBreakBefore=false,
                 :outlineLvl=-1, :contextualSpacing=false,
                 :widowControl=true, :noHyphenate=false,
                 :bookmark="", :hasBorder=false, :borderStyle="",
    ...
    func appendBlockQuote text
        if text = NULL  text = ""  ok
        aBlocks + [:type="blockquote", :text=text, :runs=[], :bookmark=""]
        return self

    func appendHyperlink label, url
        if label = NULL  label = ""  ok
        if url   = NULL  url   = ""  ok
        aBlocks + [:type="hyperlink", :text=label, :url=url, :bookmark=""]
        return self

    func appendRichParagraph runs, options
        /*  runs: list of [:text, :bold, :italic, :color, ...]
            options: list with :spaceBefore, :spaceAfter  */
        ...
        for run in runs
            ...
            nr = [:text=t,
                  ...
                  :highlight = ""]
            cv = run[:color]
            if cv != NULL  nr[:color] = cv  ok
            sv = run[:size]
            if isNumber(sv)  nr[:size] = sv  ok
            fv = run[:font]
            if fv != NULL  nr[:font] = fv  ok
            normRuns + nr
        next
        block = [:type="paragraph", :text=fullText, :runs=normRuns,
                 :align="", :spaceBefore=spBefore, :spaceAfter=spAfter,
                 :lineSpacing=0, :lineSpacingPt=0, :lineRule="",
                 :bgColor="", :indent=0, :indentRight=0,
                 :indentFirstLine=0, :keepNext=false, :keepLines=false,
                 :pageBreakBefore=false, :outlineLvl=-1,
                 :contextualSpacing=false, :widowControl=true,
                 :noHyphenate=false, :bookmark="", :hasBorder=false,
                 :borderStyle="", :borderColor="", :borderSize=0,
                 :borderSpace=0, :borderSides=[]]
        aBlocks + block
        return self

    func appendNumberedList items
        /*  items: list of strings — all appended as numbered listitem blocks.  */
        numId = 9001   # synthetic numId unlikely to clash with parsed ones
        for i = 1 to len(items)
            txt = items[i]
            if txt = NULL  txt = ""  ok
            block = [:type="listitem", :text=txt, :runs=[],
                     :listIsBullet=false, :listIlvl=0,
                     :listNumId=numId, :listStartAt=1,
                     :bookmark=""]
            aBlocks + block
        next
        return self

    func appendBulletList items
        /*  items: list of strings — all appended as bullet listitem blocks.  */
        numId = 9002
        for i = 1 to len(items)
            txt = items[i]
            if txt = NULL  txt = ""  ok
            block = [:type="listitem", :text=txt, :runs=[],
                     :listIsBullet=true, :listIlvl=0,
                     :listNumId=numId, :listStartAt=1,
                     :bookmark=""]
            aBlocks + block
        next
    ...
    func appendPageBreak
        aBlocks + [:type="pagebreak"]
        return self
    ...
    func appendTable rowsData, colWidthsCm
        ...
        blockRows = []
        isFirstRow = true
        for rawRow in rowsData
            if !isList(rawRow)  loop  ok
            blockRow = []
            for cellText in rawRow
                if cellText = NULL  cellText = ""  ok
                cell = [:text          = cellText,
                        :bgColor       = "",
                        :colspan       = 1,
                        :rowspan       = 1,
        ...
        block = [:type         = "table",
                 ...
                 :tblIndentTwips = 0,
                 :rowBgColors  = [],
                 :tblLookFirstRow = false,
                 :tblLookLastRow  = false,
                 :tblLookFirstCol = false,
                 :tblLookLastCol  = false,
                 :tblLookNoHBand  = false,
                 :tblLookNoVBand  = false]
        aBlocks + block
        return self
